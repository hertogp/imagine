...
- `mkdir -p ~/lib/python`
- `cp -R lib/* ~/lib/python`
- add ~/lib/python to your PYTHONPATH
- python script, save output to filename given by `sys.argv[-1]`
- image format should match `im_fmt` used to create a link to the image.

\newpage

# [*ChartDirector*](http://www.advsofteng.com/index.html)

The yellow bars in the images created by ChartDirector are because this
is the demo-version without a license.

## Line Chart

```{.shebang caption="Created by ChartDirector"}
#!/usr/bin/python
import os, sys
...
```{.shebang im_fmt="pdf" width="120%"}
#!/usr/bin/env python
import sys  # required in order to pickup filename from sys.argv
from pychartdir import *
...
See [*ChartDirector*](http://www.advsofteng.com/index.html) and the `doc`
