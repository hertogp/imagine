...
class SheBang(Handler):
    ...
    def image(self, fmt=None):
        ...
        args = self.options.split() + [self.outfile]
        if self.cmd(self.inpfile, *args):
            rv = self.Para()
            if not os.path.isfile(self.outfile):
                rv = self.AnonCodeBlock()
                self.msg(1, '>>:', 'missing', self.outfile)
            if len(self.output):
                out = pf.CodeBlock(self.codec[0], self.output)
                rv = rv + [out] if hasattr(rv, 'append') else [rv, out]
            return rv
